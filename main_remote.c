...
static volatile THE_MESSAGE counter = {0};
static volatile THE_MESSAGE temp = {0};
...
SemaphoreHandle_t xsemaphorebin;
...
static void temperature_task(void *param)
{
    ...
    CLOCK_AttachClk(kFRO_HF_to_ADC_CLK); /*!< Switch ADC_CLK to FRO */
...
static void app_task(void *param)
{
    ...
    while (counter.DATA <= 100U)
    {
        (void)PRINTF("Waiting for ping...\r\n");
        (void)rpmsg_queue_recv(my_rpmsg, my_queue, (uint32_t *)&remote_addr, (char *)&counter, sizeof(THE_MESSAGE),
                               ((void *)0), RL_BLOCK);
        if(counter.DATA > 0 && counter.DATA % 5 == 0)
        {
        	xSemaphoreGive(xsemaphorebin);
        	(void)PRINTF("Sending Temperature...\r\n");
			temp.DATA = g_CurrentTemperature;
            (void)rpmsg_lite_send(my_rpmsg, my_ept, remote_addr, (char *)&temp, sizeof(THE_MESSAGE), RL_BLOCK);

        }
        else
        {
        	(void)PRINTF("Sending counter echo...\r\n");
            (void)rpmsg_lite_send(my_rpmsg, my_ept, remote_addr, (char *)&counter, sizeof(THE_MESSAGE), RL_BLOCK);
        }
...
int main(void)
{
    ...
    if (xTaskCreate(temperature_task, "temp_task", APP_TASK_STACK_SIZE, NULL, tskIDLE_PRIORITY + 1U, &app_task_handle) != pdPASS)
       {
