...
static THE_MESSAGE counter = {0};
static THE_MESSAGE temp = {0};
...
static void app_task(void *param)
{
    ...
    counter.DATA = 0U;
    (void)rpmsg_lite_send(my_rpmsg, my_ept, remote_addr, (char *)&counter, sizeof(THE_MESSAGE), RL_DONT_BLOCK);
    ...
    while (counter.DATA <= 100U)
    {
        ...
        {
            (void)PRINTF("Message: Size=%x, TEMP = %i\r\n", len, temp.DATA);

        }
        else
        {
            (void)PRINTF("Message: Size=%x, COUNTER = %i\r\n", len, counter.DATA);
        }
        counter.DATA++;
